# Databricks notebook source exported at Fri, 8 Jul 2016 01:21:50 UTC
# MAGIC %md
...
pluralDF = ( wordsDF
            .select( ( concat( wordsDF.word, lit('s') ) ).alias('word') )
           )
...
def removePunctuation(column):
    ...
    """
    return trim( lower( regexp_replace(column, "\p{Punct}", "") ) )
...
(sentenceDF
 .select(removePunctuation(col('sentence')).alias('words'))
 .show(truncate=False))
...
shakeWordsDF = ( shakespeareDF
                .select( (explode(split(shakespeareDF["trim(lower(regexp_replace(value,\p{Punct},)))"]," "))).alias('word') )
                .where("word != ''")
                )
