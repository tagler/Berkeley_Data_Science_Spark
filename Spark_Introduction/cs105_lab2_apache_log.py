# Databricks notebook source exported at Sat, 9 Jul 2016 23:16:09 UTC
# MAGIC %md
...
days_with_hosts, hosts = zip(*data)
...
combined_df = ( total_req_per_day_df
               .join( daily_hosts_df, ["day"] )
               .withColumnRenamed("count","hosts_count")
               )

avg_daily_req_per_host_df = ( combined_df
                             .select("day", (combined_df.total_count / combined_df.hosts_count).alias("avg_reqs_per_host_per_day") )
                             )
...
data_2 = avg_daily_req_per_host_df.collect()
days_with_avg, avgs = zip(*data_2)

days_with_avg = list(days_with_avg)
avgs = list(avgs)
...
not_found_paths_df = ( not_found_df
                      .select( "path" )
                      )
...
data_3 = errors_by_date_sorted_df.collect()
days_with_errors_404, errors_404_by_day = zip(*data_3)

days_with_errors_404 = list(days_with_errors_404)
errors_404_by_day = list(errors_404_by_day)
...
top_err_date_df = ( errors_by_date_sorted_df
                   .orderBy(desc("count"))
                   )
...
data_4 = hour_records_sorted_df.collect()
hours_with_not_found, not_found_counts_per_hour = zip(*data_4)

hours_with_not_found = list(hours_with_not_found)
not_found_counts_per_hour = list(not_found_counts_per_hour)
