# Databricks notebook source exported at Thu, 1 Sep 2016 01:58:45 UTC
# MAGIC %md
...
def tokenize(string):
    ...
    """
    all_words = simpleTokenize(string)
    
    saved_words = []
    for each_word in all_words:
      if each_word not in stopwords:
        saved_words.append(each_word)
...
def countTokens(vendorRDD):
    ...
    """
    vendorRDD_tokens = vendorRDD.flatMap(lambda (x,y) : (y))
    return vendorRDD_tokens.count()
...
def findBiggestRecord(vendorRDD):
    ...
    """    
    return vendorRDD.takeOrdered(10, key = lambda (x,y) : -len(y))
...
def tf(tokens):
    ...
    for each in tokens:
      if each in dic:
        ...
        dic[each] = 1
    # divide each value by number of tokens
    for key,value in dic.items(): dic[key] = value/ float(len(tokens))
...
smallIDFTokens = idfsSmall.takeOrdered(11, key = lambda (x,y) : y)
...
small_idf_values = idfsSmall.map(lambda (x,y): (y)).collect()
fig = plt.figure()
plt.hist(small_idf_values, 20)
display(fig)
...
idfsToCountRow = idfsSmall.toDF(['word','idf']).select("idf")
display(idfsToCountRow)
...
avgSimDups = trueDupsRDD.map(lambda x : float(x[1][0])).mean()

nonDupsRDD = ( sims
              .subtractByKey(trueDupsRDD)
              )

avgSimNon = nonDupsRDD.map(lambda x : x[1]).mean()
...
def invert(record):
    ...
    """
    pairs = []
    for (token, weight) in record[1].items():
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for token in tokens:
      p1 = amazonWeightsBroadcast.value[amazonRec][token]
      p2 = googleWeightsBroadcast.value[googleRec][token]
      s = s + (p1 * p2)
    
    d1 = amazonNormsBroadcast.value[amazonRec]
    d2 = googleNormsBroadcast.value[googleRec]
    value = s / (d1 * d2)
