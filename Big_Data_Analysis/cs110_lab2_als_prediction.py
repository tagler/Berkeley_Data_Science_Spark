# Databricks notebook source exported at Tue, 23 Aug 2016 05:08:07 UTC
# MAGIC %md
...
movies_with_500_ratings_or_more.show(10)
...
avg_rating_df = training_df.select( mean("rating") )
...
my_unrated_movies_df = ( not_rated_df
                        .select( not_rated_df.ID.alias("movieId"), 
                                not_rated_df.title, 
                                lit(0).alias("userId") )
                        )
...
predicted_with_counts_df = ( predicted_ratings_df
                            .join(movie_names_with_avg_ratings_df,
                                 predicted_ratings_df.movieId == movie_names_with_avg_ratings_df.movieId,
                                 "inner")
                            .drop(predicted_ratings_df.title)
                            .drop(predicted_ratings_df.movieId)
                            )
