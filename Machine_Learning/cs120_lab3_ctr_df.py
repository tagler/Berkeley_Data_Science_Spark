# Databricks notebook source exported at Thu, 28 Jul 2016 03:26:34 UTC
# MAGIC %md
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    i = []
    for x in raw_feats:
      i.append( ohe_dict_broadcast.value[x] )
      
    return SparseVector( num_ohe_feats, np.sort(i), np.ones( len(i) ) )
...
sample_ohe_df = sample_data_df.select( (sample_ohe_dict_udf("features")).alias("values") )
...
def parse_raw_df(raw_df):
    ...
    """
    final = ( raw_df
             .select( split(raw_df.text, ',')[0].cast("double").alias("label"),
                      parse_point_udf(raw_train_df.text).alias("feature") )
            )
    return final
...
ohe_train_df = ( parsed_train_df
                 .select( "label", (ohe_dict_udf("feature")).alias("features") )
                 .cache()
               )
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    i = []
    for x in raw_feats:
      if ohe_dict_broadcast.value.has_key(x):
        i.append( ohe_dict_broadcast.value[x] )
...
ohe_validation_df = ( parsed_validation_df
                       .select( "label", ( ohe_dict_missing_udf("feature") ).alias("features") )
                       .cache()
                   )
...
lr = LogisticRegression(regParam=reg_param, elasticNetParam=elastic_net_param, standardization=standardization, maxIter=max_iter)
...
def add_log_loss(df):
    ...
    """
    df2 = df.select( (when(df.p == 1.0, df.p-epsilon).when(df.p == 0.0, df.p+epsilon).otherwise(df.p) ).alias("p_mod"), "p", "label")
    df3 = df2.select( "p", "p_mod", "label", when(df2.label == 1, -log(df2.p_mod)).otherwise(-log(1-df2.p_mod)).alias("log_loss") )
    return df3
...
new_df = add_log_loss( ohe_train_df
                      .select( "label", "features", lit(class_one_frac_train).alias("p") )
                      )

log_loss_tr_base = ( new_df
                    .select( avg("log_loss") )
                    .first()[0]
                    )
...
def add_probability(df, model):
    ...
    def get_p(features):
        ...
        if raw_prediction < -20:
          raw_prediction = -20
        if raw_prediction > 20:
          raw_prediction = 20
        # Return the probability
        return (1.0/(1+exp( -raw_prediction )))
...
def add_hashed_features(df):
    ...
    final = (df
              .select("label",
                      tuples_to_hash_features_udf(df.feature).alias("features") )
            )
    return final
...
def get_sparsity(df):
    ...
    """
    df_1 = df.select( feature_sparsity_udf(df.features).alias("features") )
    return df_1.select(avg("features")).first()[0]
...
lr_hash = LogisticRegression(regParam=reg_param, elasticNetParam=elastic_net_param, standardization=standardization, maxIter=max_iter)
