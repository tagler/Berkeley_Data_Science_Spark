# Databricks notebook source exported at Fri, 22 Jul 2016 03:24:19 UTC
# MAGIC %md
...
def parse_points(df):
    ...
    result = ( df
              .select(split('value', ','))
              .rdd
              .map(lambda values : LabeledPoint( values[0][0], values[0][1:]) )
              .toDF()
              )
    return result
...
min_year = float( content_stats.rdd.collect()[3][1] )
max_year = float( content_stats.rdd.collect()[4][1] )
...
parsed_data_df = ( parsed_points_df
                   .select( "features", parsed_points_df['label'] - 1922 )
...
average_train_year = float( parsed_train_data_df
                          .describe()
                          .rdd
                          .collect()[1][1]
                         )
...
preds_and_labels_train = parsed_train_data_df.select("label").rdd.map(lambda x : (average_train_year, x[0]) )
...
preds_and_labels_val = parsed_val_data_df.select("label").rdd.map(lambda x : (average_train_year, x[0]) )
...
preds_and_labels_test = parsed_test_data_df.select("label").rdd.map(lambda x : (average_train_year, x[0]) )
...
preds_and_labels = ( parsed_val_data_df
                      .rdd
                      .map(lambda x: ( float(x[0].dot(weights_LR0)), float(x[1]) ) )
                    )
...
lin_reg = LinearRegression(featuresCol="features", labelCol="label", maxIter=num_iters, regParam=reg, elasticNetParam=alpha, fitIntercept=use_intercept )
...
sample_prediction = first_model.transform(parsed_train_data_df).select("prediction")
...
val_pred_df = first_model.transform(parsed_val_data_df).select("label","prediction")
...
def two_way_interactions(lp):
    ...
    combos = list(itertools.product(lp.features, lp.features))
    vals = len(combos)
    products = []
    for x in combos:
      temp = x[0] * x[1]
      products.append(temp)
    new_features = np.hstack((lp.features,products))
    
    return LabeledPoint( lp.label, new_features  )
    
# Transform the existing train, validation, and test sets to include two-way interactions.
# Remember to convert them back to DataFrames at the end.
train_data_interact_df = parsed_train_data_df.rdd.map(lambda x : two_way_interactions(x)).toDF(['features','label'])
val_data_interact_df = parsed_val_data_df.rdd.map(lambda x : two_way_interactions(x)).toDF(['features','label'])
test_data_interact_df = parsed_test_data_df.rdd.map(lambda x : two_way_interactions(x)).toDF(['features','label'])
